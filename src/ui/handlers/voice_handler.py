...
class VoiceHandler:
    ...
    def process_asr_result(self, text):
        ...
        # [A63] Step-by-step logging to pinpoint exactly where output gets blocked
        if not text or not text.strip():
            logger.info(f"🎙️ [VOICE] Gate-1: Empty/no-speech ASR result, skipping.")
            return
        ...
        if not re.search(r'[\u4e00-\u9fffa-zA-Z0-9]', clean_text):
            logger.info(f"🎙️ [VOICE] Gate-2: Bypassed non-speech/noise ASR result: '{clean_text}'")
            return
        ...
        try:
            ...
            logger.error(f"❌ [VOICE] Learning engine apply_habits failed: {le}, using original text")
            learned = clean_text

        # [A63] CRITICAL NULL GUARD: apply_habits might return None in edge cases
        if learned is None:
            logger.warning(f"⚠️ [VOICE] Gate-3: apply_habits returned None, fallback to original: '{clean_text}'")
            learned = clean_text
        ...
        punc_pattern = r'[，。！？：；「」『』、（）—－\s,\.!\?()[\]{};:\-_""]'
        effective_text = re.sub(punc_pattern, '', learned) if learned else ''
        if len(effective_text) <= 3:
            learned = effective_text
            
        if not learned:
            logger.info(f"🎙️ [VOICE] Gate-4: Text became empty after processing, skipping. Original: '{clean_text}'")
            return
        
        # [A18] Block single character if it is "我" or "嗯"
        if learned in ["我", "嗯"]:
            logger.info(f"🎙️ [VOICE] Gate-5: Bypassed single word noise: '{learned}'")
            return
        
        # 3. DIRECT OUTPUT (Zero-Latency Pipe)
        logger.info(f"🎙️ [VOICE] Gate-6: Sending to output: '{learned}'")
